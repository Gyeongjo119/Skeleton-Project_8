    ...
    <div class="container mt-3 p-1">
      ...
      <form>
        ...
        <div>
          ...
          <div class="d-flex flex-row">
            <div
              v-for="(category, index) in expenseCategory[0]"
              :key="'expense-' + index"
              class="form-check"
            >
              <input
...
const { transactions, incomeCategory, expenseCategory } = storeToRefs(store);
...
watch(
  ...
  ([newIncomeCategory, newExpenseCategory]) => {
    if (newIncomeCategory && newIncomeCategory[0]) {
      selectedIncomeCategories.value = [...newIncomeCategory[0]];
    }
    if (newExpenseCategory && newExpenseCategory[0]) {
      selectedExpenseCategories.value = [...newExpenseCategory[0]];
    }
  },
